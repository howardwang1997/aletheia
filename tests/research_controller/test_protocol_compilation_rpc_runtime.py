...
from aletheia.research_controller_protocol_compilation_runtime import (
...
from test_protocol_template_provider import _fixture as _provider_fixture  # noqa: E402
...
def _kernel_reader(root: Path) -> ResearchKernelReadOnlyConfig:
    ...
    root.chmod(0o550)
    metadata = root.stat()
    public_key = _public_key("protocol-compilation-kernel-root")
    return ResearchKernelReadOnlyConfig(
        trust_root=ResearchAuthorizationTrustRootV1(
            trust_root_id=f"rat_{_sha('protocol-compilation-kernel-root')[:32]}",
            frozen_at=NOW,
            commissioning_keys=(
                ResearchAuthorizationTrustKey(
...
def _fixture(tmp_path: Path):
    ...
    factory = (
        repository_root / "aletheia/research_controller_protocol_compilation_runtime.py"
    ).resolve()
    implementation = (
        repository_root / "aletheia/research_controller/protocol_template_provider.py"
    ).resolve()
    context, _request, compilation_policy, provider_policy, _provider = _provider_fixture()
    binding = ControllerStepAuthorityBinding(
        role=ControllerStepAuthorityRole.PROTOCOL_COMPILATION,
        principal_id="service:protocol-compilation",
        key_id=None,
        policy_sha256=compilation_policy.policy_sha256,
        service_manifest_sha256=_sha("protocol-compilation-service-manifest"),
        externally_deployed=True,
    )
    receipt_private_key = Ed25519PrivateKey.from_private_bytes(
        hashlib.sha256(b"protocol-compilation-rpc-receipt").digest()
    )
    ...
    pin = ControllerWorkerRPCServicePin(
        ...
        socket_path=str(socket_root / "protocol-compilation.sock"),
    ...
    assert implementation_sha256 == provider_policy.provider_implementation_sha256
    config = {
        ...
        "compilation_policy": compilation_policy.model_dump(mode="json"),
    ...
    config_path = (config_root / "protocol-compilation.json").resolve()
    ...
    deployment = ControllerWorkerRPCServerDeployment(
        ...
        composition_factory_module="aletheia.research_controller_protocol_compilation_runtime",
        composition_factory_attribute="build_protocol_compilation_rpc_service",
    ...
    return deployment, config, config_path, context


def test_checked_in_protocol_compilation_factory_is_operation_closed(tmp_path: Path) -> None:
    deployment, _config, config_path, _context = _fixture(tmp_path)
    handlers = build_protocol_compilation_rpc_service(
        deployment=deployment,
        configuration_bytes=config_path.read_bytes(),
    )

    assert handlers.operations == (ControllerWorkerRPCOperation.COMPILE_PROTOCOL,)
    handler = handlers.handler_for(ControllerWorkerRPCOperation.COMPILE_PROTOCOL)
    with pytest.raises(TypeError, match="another payload"):
        handler(object())


def test_guarded_rpc_runtime_loads_exact_protocol_compilation_factory(tmp_path: Path) -> None:
    deployment, _config, _config_path, _context = _fixture(tmp_path)
...
def test_protocol_compilation_factory_rejects_duplicate_or_rebound_config(
    tmp_path: Path,
) -> None:
    deployment, config, config_path, _context = _fixture(tmp_path)
    ...
    config["provider_policy"]["provider_implementation_sha256"] = _sha("rebound-source")
