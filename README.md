...
proposal steps now re-audit the exact Kernel/CAS and
blocked-compilation or incorporated-observation/continuation chain, restrict model output to a
closed draft, and atomically retain one unsigned first-writer-wins proposal per audited request.
They return only `awaiting_authority`; no controller proposal service holds a Kernel signing key or
mutates the ledger. The protocol-compilation step now re-audits the exact authorized Kernel/CAS
...
PR-7j service now closes that next boundary without importing the SEA key: a new registration locks
and replays the exact current `AUTHORIZED` Kernel head inside the same PostgreSQL transaction that
appends the SEA and performs PR-4 qualification admission/reservation. Exact retries return the same
committed-state receipt even after attempt progress; one-sided historical SEA/reservation state is
rejected, and the former standalone SEA writer is removed. The process exposes no launch, terminal,
validation, admission, or Kernel operation.
PR-7k closes the following read boundary: a single-operation, keyless raw-run source re-verifies the
...
it is never promoted to scientific evidence.
PR-7n closes the committed-validation read boundary: a keyless single-operation source resolves the
exact durable slot row and re-verifies its DB signature, independent validation, F9-v2 campaign,
Kernel action, PR-4 lineage and artifact bytes before returning it. Caller material cannot replace
the stored receipt.
PR-7o closes the independent-admission boundary: one isolated process owns only the admitter key,
re-verifies the complete committed-validation and live DB-challenge chain, and mechanically signs
either an admitted confirmation or a rejection carrying the validation blockers. Decision time is
sampled from PostgreSQL inside the challenge's half-open window, preventing expired challenge
replay. The proposal cannot fill the slot or mutate the Kernel. One concrete service factory
remains: atomic admission/Kernel incorporation.
The
...
[ADR 0078](docs/architecture/0078-qualification-terminal-outbox-factory.md), and
[ADR 0079](docs/architecture/0079-disabled-qualification-host-bootstrap.md).
