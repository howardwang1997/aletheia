...
async def main(timestamp: str) -> None:
    ...
    exp_id = finalize_plan(
        run_id,
        {
            ...
            "elemental co-occurrence (NOT from Tc), so the test is non-circular and not "
            "applicability-domain. The single test statistic is the cuprate-minus-matched-control MAE "
            "excess on a held-out confirm split, with a permuted-strata null. The mechanism (cuprate Tc "
            ...
            "HARD DESIGN LOCK — use EXACTLY this high-power form (prior rejections were all "
            "UNDERPOWERED, not un-novel): use ALL cuprates in the stratum (NO Tc-window restriction), "
            "the statistic is mean ABSOLUTE error |y_hat - y|, the control is non-cuprates matched ONLY "
            "on element count + Magpie feature-space density (nearest-neighbor, ~equal size, hundreds of "
            "pairs), significance by a matched bootstrap CI lower bound > 0 plus a permuted-strata null. "
            "Maximize matched sample size. Do NOT add ANY of: signed-bias decomposition; an aleatoric / "
            ...
            "lacks and have been REJECTED for low power. Keep it to the one high-power matched-control "
            ...
            "repeated-split bootstrap-CI lower bound > 0). Use the HIGH-POWER form: ALL cuprates, mean "
            "ABSOLUTE error, matched ONLY on element count + feature-space density, hundreds of pairs. "
            ...
            "framing. (All of those are UNDERPOWERED on this data and have been rejected.) It is a "
            "modest DIAGNOSTIC of a model error gap, not a paradigm shift. Matching is DETERMINISTIC (seeded).",
