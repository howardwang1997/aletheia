...
Entry points for the live research loop: **live e2e runners**, the **cheap probe pipeline** that
de-risks a discriminating effect before an expensive run, and **utilities**. This page covers the
research-loop scripts; the deployment, qualification, and controller-runtime scripts
(`durable_worker.py`, the RPC service runners, the `author-*`/`commission-*`/`prepare-*` set) are
documented in the corresponding `docs/PR*.md` guides and operator runbooks.
...
| script | what it runs |
|---|---|
| `real_k2_campaign_e2e.py` | the K2 multi-round campaign runner: the cuprate-Tc plane-doping diagnostic on UCI superconductivity. Prints a K2 ✓/✗ checklist + verdict. |
