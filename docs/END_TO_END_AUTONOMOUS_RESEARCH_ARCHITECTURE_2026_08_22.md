...
认证：PR-8a 已补五个 one-role runner 及 systemd `ExecStart` 的 exact manifest SHA pin，但尚无 production
service factory set、target-host installer、concrete observer、frozen manifest instance 或 campaign runner，
...
锁定并重放 exact Kernel/CAS 与 durable compilation/continuation receipts，只把封闭的 provider choices
...
### PR-7l：Database-observation attestation RPC service

- 单 service 只暴露 validation challenge、validation commit 与 admission challenge 三项 operation，并且只
  加载独立 database-attestation domain key；validator、admitter、execution 与 Kernel private key 均不存在；
- 每项 operation 先 `FOR UPDATE` 锁定 exact preregistered SEA slot，再采样 PostgreSQL time；validation commit
  必须引用数据库已 append 的 exact historical challenge，不能使用仅签名但未注册的 caller challenge；
- 完整 PR-4/Kernel/raw-run/F9-v2/signature 验证结束后再次采样 DB time，过期 challenge 不得越过 commit
  linearization；admission challenge 签发前重验 exact durable committed validation；
- 三种 RPC receipt 都是 closed Pydantic contract，exact retry 不包含 transient `created` 状态；
- guarded factory 冻结 database/schema、service/domain key、read-only Kernel CAS、完整 PR-4 lineage/artifact
  custody、read-only F9-v2 campaign archive、authority separation 与 implementation bytes。

该切片关闭 database-attestation source composition，不等于 PostgreSQL ACL、host key custody 或 live
process-kill evidence；在 PR-7l checkpoint 尚余四项 factory，PR-7m 随后关闭其中的 independent F9-v2
validation。详见 `PR7L_DATABASE_OBSERVATION_SERVICE.md` 与
`architecture/0069-database-observation-rpc-service.md`。

### PR-7m：Independent F9-v2 validation RPC service

- 双 operation service 只负责 prepare validation campaign 与 issue validation receipt，并且只加载
  observation-validator domain key；RPC receipt key 独立，database/admission/execution/Kernel private key 均不存在；
- 每次 assessment 前重放 exact Kernel action、完整 PR-4 qualification/run lineage 与 fresh artifact CAS，request
  精确绑定 graph scope、Protocol IR、F9-v2 world model/predictions、scientific slot、namespace 与 commitment；
- checked-in conservative assessor 只接受 frozen catalog 中完整 scope/schema/content digest 的 known-answer
  mapping；unknown/rebound content typed-block，不猜测 positive/negative/inconclusive；
- campaign archive 是 process-owned `0700` write-once root，每个 raw run 只有一个 immutable winner；receipt
  issuance fresh-rehash winner，并只用 database public pin 验证 DB issuance challenge；
- guarded factory 冻结 database/schema、read-only Kernel/PR-4/artifact roots、writable campaign root、全部 public
  authorities、service/assessor source bytes、validator `0400` key 与 authority separation。

该切片关闭 independent-validator source composition 和 known-answer deterministic baseline，不等于 general
domain assessor、host ACL/key custody 或 live process-kill evidence；在 PR-7m checkpoint 尚余三项 factory，
PR-7n 随后关闭其中的 committed-validation source。详见 `PR7M_INDEPENDENT_F9_V2_VALIDATION_SERVICE.md` 与
`architecture/0070-independent-f9-v2-validation-rpc-service.md`。

### PR-7n：Committed-validation source RPC service

- 单 operation keyless service 只接受 Quest/action/scientific-slot identity，不接受 caller-supplied receipt、
  campaign、terminal material 或 time；
- source 从 append-only row 重建 exact `ObservationValidationReceiptWrite`，canonical JSON/hash/action 任一
  rebound 都 fail closed；
- 在 PostgreSQL time 下历史重验 DB challenge/commit、validator receipt、F9-v2 campaign、Kernel action/snapshot、
  signed SEA、完整 PR-4 qualification/run lineage 与 fresh artifact CAS；
- service principal/manifest/policy 与其验证的 database/validator authority bindings 分离，只持 RPC transport
  key，不加载任何 domain private key；
- guarded factory 冻结 database/schema、adapter bytes、read-only Kernel/PR-4/artifact/registry/F9 archive roots
...
### PR-7o：Independent admission-decision RPC service

- 单 operation service 只持 observation-admitter domain key 与独立 RPC receipt key；database、execution、
  validator 与 Kernel private key 均不得加载；
- 输入只能是 exact committed validation 与 DB-signed admission challenge；signer 自行重放 Kernel action、
  signed SEA、完整 PR-4 lineage、fresh artifact CAS、F9-v2 campaign、validator 与 DB signatures；
- `VALIDATED_CONFIRMATION` 机械映射为无 blocker 的 `ADMITTED` proposal，verified scientific rejection 或
  engineering blocker 机械映射为保留原 blocker 的 `REJECTED` proposal；caller 不能选择 disposition/reason；
- decision 不再回填 challenge issuance time：service 在完整 custody replay 前后采样 PostgreSQL time、拒绝
  clock rollback，并强制
  `issued_at <= decided_at < expires_at`；后续 atomic commit 还必须满足
  `decided_at <= registered_at <= committed_at < expires_at`；
...
源码/配置 custody 和 live Linux UID/GID；它不提供 production factory、安装或启动权限。下一项顺序工作是
实现 opt-in、crash-safe target-host installer，再进入真实 image/host qualification 与
