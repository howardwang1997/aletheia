...
watchdog 与 node cleanup 在删除前分别重验相同 container ID/name、完整 state/timestamps、冻结 OCI
enforcement projection 与 fresh seccomp-copy hash，而不把无关的当前 Docker metadata 漂移误作权限变化。
历史完整 inspection hash 仍保持不可变；该分支不生成 runtime identity、工程成功或科学证据，
