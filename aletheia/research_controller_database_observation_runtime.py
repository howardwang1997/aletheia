"""Guarded-loader factory for the database scientific-observation RPC service."""
...
def build_database_observation_rpc_service(*, deployment, configuration_bytes):
    ...
    class DatabaseObservationSigningKeyPin(BaseModel):
        ...
        def _path_is_canonical(self):
            ...
            if (
                ...
                raise ValueError("database observation signing key path is not canonical")
            return self
    ...
    class F9V2ValidationArchiveReadConfig(BaseModel):
        ...
        def _root_is_canonical_and_readable(self):
            ...
            if (
                ...
                raise ValueError("F9-v2 validation archive read root is not closed")
            return self

    class DatabaseObservationRPCConfig(BaseModel):
        model_config = ConfigDict(extra="forbid", frozen=True)

        schema_name: Literal["aletheia.database_observation_rpc_service_config"] = (
            "aletheia.database_observation_rpc_service_config"
        )
        ...
        database_signing_key: DatabaseObservationSigningKeyPin
        ...
        def _authority_is_closed(self):
            ...
            if (
                binding.role is not ControllerStepAuthorityRole.DATABASE_ATTESTATION
                ...
                or not database.active_at(self.prepared_at)
                or not all(
                ...
                raise ValueError("database observation RPC authority is not closed")
            return self
    ...
    def unique_object(pairs):
        ...
        if duplicates:
            raise ValueError(f"duplicate database observation RPC config keys: {duplicates}")
        return dict(pairs)
    ...
    try:
        ...
        config = DatabaseObservationRPCConfig.model_validate(raw)
    except (TypeError, ValueError) as exc:
        raise ValueError("database observation RPC config is invalid") from exc
    if canonical_json_bytes(config) != configuration_bytes:
        raise ValueError("database observation RPC config is not canonical JSON")
    ...
    if (
        ...
        or database.principal_id in {item.principal_id for item in kernel_keys}
        or database.key_id in {item.key_id for item in kernel_keys}
        ...
        or config.database_signing_key.owner_gid != deployment.process_gid
    ):
        raise ValueError("database observation RPC config differs from deployment or authority")

    reviewed_root = Path(deployment.reviewed_code_root)
    implementation_path = Path(config.service_implementation_source_path)
    ...
    try:
        ...
        raise ValueError("database observation implementation escaped reviewed source") from exc
    if implementation_path != expected_module_path:
        raise ValueError("database observation implementation resolved another module")
    custody_roots = (
    ...
    for index, first in enumerate(custody_roots):
        for second in custody_roots[index + 1 :]:
            if first == second or first in second.parents or second in first.parents:
                raise ValueError("database observation custody roots overlap")

    before = fresh_regular_bytes(
        implementation_path,
        expected_sha256=config.service_implementation_source_sha256,
        label="database observation implementation",
    )
    private_key = fresh_regular_bytes(
        ...
        expected_owner=(
            ...
            config.database_signing_key.owner_gid,
        ),
        expected_mode=config.database_signing_key.file_mode,
        label="database observation signing key",
    )
    public_hex = (
    ...
    if (
        public_hex != database.public_key_ed25519_hex
        or scientific_bridge_key_id(public_hex) != config.database_signing_key.key_id
    ):
        raise ValueError("database observation private key differs from its public pin")
    ...
    exact_directory(
        cas_path,
        label="database observation Kernel CAS",
    ...
    exact_directory(
        archive_path,
        label="database observation F9-v2 archive",
    ...
    service = service_module.PostgreSQLScientificBridgeService(
        verification=ObservationAdmissionVerificationContext(
            ...
            database_private_key=private_key,
        ),
        challenge_ttl=timedelta(seconds=config.challenge_ttl_seconds),
    )

    def issue_validation_challenge(payload):
        if type(payload) is not ValidationChallengeIssuanceRPCPayload:
            raise TypeError("database observation RPC handler received another payload type")
        return service.issue_validation_challenge(
            raw_run=payload.raw_run,
            validation_campaign_sha256=payload.validation_campaign_sha256,
        )

    def commit_validation(payload):
        if type(payload) is not ValidationCommitRPCPayload:
            raise TypeError("database observation RPC handler received another payload type")
        return service.commit_validation(payload.receipt)

    def issue_admission_challenge(payload):
        if type(payload) is not AdmissionChallengeIssuanceRPCPayload:
            raise TypeError("database observation RPC handler received another payload type")
...
__all__ = ["build_database_observation_rpc_service"]
